...
import { getModeColor, getModeBg } from '@/lib/theme-colors'
...
function Skills() {
  ...
  const filteredSkills = useMemo(() =>
    mode ? skills.filter(s => s.modes.includes(mode)) : skills,
    [mode]
  )
  ...
  const modeBgColor = getModeBg(mode)
  const modeTextColor = getModeColor(mode)
      ...
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <div className="text-center mb-12 md:mb-16">
            <p className={`text-[10px] md:text-xs font-bold tracking-[0.3em] uppercase ${modeTextColor} mb-4`}>
              Expertise
            </p>
            ...
            {Object.entries(groupedSkills).map(([category, categorySkills], categoryIndex) => (
              ...
              >
                <h3 className="text-sm font-bold text-white/80 mb-5 flex items-center gap-3">
                  <span className={`w-1 h-4 ${modeBgColor} rounded-full`} />
                  ...
                  {categorySkills.map((skill, index) => (
                    ...
                    >
                      ...
                      <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <motion.div
                          className={`h-full ${modeBgColor} rounded-full`}
                          initial={{ width: 0 }}
